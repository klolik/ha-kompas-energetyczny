...
async def async_setup_entry(
    ...
    entities.extend(
        [
            KompasEnergetycznyPowerGenerationShareSensor(api_data, **cfg)
            for cfg in sensors
            if cfg["key"] not in ["generacja", "zapotrzebowanie"]
        ]
    )
    entities.append(
        KompasEnergetycznyPowerConsumptionShareSensor(
            api_data, "generacja", "Consumption"
        )
    )
...
class KompasEnergetycznyBaseSensor(SensorEntity):
    ...
    def __init__(
        self, api_data: KompasEnergetycznyApiData, src: str, sid: str, name: str
    ) -> None:
...
class KompasEnergetycznyPowerSensor(KompasEnergetycznyBaseSensor):
    ...
    def __init__(
        self, api_data: KompasEnergetycznyApiData, key: str, name: str
    ) -> None:
    ...
    def native_value(self):
        podsumowanie = self.api_data.coordinator.data.get("data", {}).get(
            "podsumowanie", {}
        )
...
class KompasEnergetycznyStatusSensor(KompasEnergetycznyBaseSensor):
    ...
    def get_znacznik(self):
        ...
        for item in pdgsz:
            if (
                datetime.fromisoformat(item.get("dtime")).astimezone(
                    dateutil.tz.tzlocal()
                )
                == now
            ):
